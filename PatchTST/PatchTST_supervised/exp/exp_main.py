        ...
        with torch.no_grad():
            for n in range(len(self.test_loader_list)):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    if i % 1 == 0:
                        ...
                        if i == 0:
                            ...
                            batch_all = np.concatenate((batch_all,input[:,3:,:]),axis=1)
                        # print(input.shape)
                        # print(input[0, :, -1].shape)
                        # print(pred[0, :, -1].shape)
                        # print(i)
                        ...
                        v1_list.append(batch_all[0, :, 0])
                        v2_list.append(batch_all[0, :, 1])
                        v3_list.append(batch_all[0, :, 2])
                        v4_list.append(batch_all[0, :, 3])
                        v5_list.append(batch_all[0, :, 4])

                batch_all = np.concatenate((batch_all[0, :, -1],true[0, :, -1]),axis=0)

                gt = batch_all
                # print(pd_list[-1].shape)
                print("Rehab evaluation score:",np.abs(gt[-1]-pd_list[-1][-1])/gt[-1])
                # gt = batch_all[0, :, -1]
                for iii in range(len(pd_list)):
                    ...
                    # visual2(gt,iii+1,v1,v2,v3,v4,v5,pd, os.path.join(folder_path, str(n)+'-'+str(iii) + '.jpg'))
